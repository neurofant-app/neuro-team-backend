...
public class ProxySeguridad : IProxySeguridad
{
    ...
    public ProxySeguridad(IOptions<ConfiguracionAPI> options, ILogger<ProxySeguridad> logger, IServicioAutenticacionJWT autenticacionJWT, IHttpClientFactory httpClientFactory)
    {
        configuracionAPI = options.Value;
        host = configuracionAPI.ObtieneHost("seguridad");
        this.logger = logger;
    ...
    {
        string logInfo = ExtensionesLog.InfoMetodo().PrefijoLog();
        if (host == null)
        {
            logger.LogError("{0} {1}", logInfo, "No hay configuración para el Host de seguridad");
            return;
        }
        ExtensionesLog.PrefixDebug(parametros: new object[3]
        {
            "A",
            DateTime.Now,
            apps
        }, logger: logger, texto: "ejemplo");
        logger.PrefixDebug("MI texto");
        ActualizaHeaders("mi-dominio", "x-uo-id");
        try
        {
            _ = new object[2] { "apps", "more" };
            ...
            TokenJWT jWT = await autenticacionJWT.TokenInterproceso(host.ClaveAutenticacion);
            if (jWT == null)
            {
            ...
            foreach (Aplicacion app in apps)
            {
                ...
                StringContent payload = new StringContent(JsonConvert.SerializeObject(app, new JsonSerializerSettings
                {
                ...
                HttpResponseMessage response = await seguridadHttpClient.PutAsync(rutaAppId, payload);
        ...
        try
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    seguridadHttpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", jWT.access_token);
                    HttpResponseMessage response = await seguridadHttpClient.GetAsync($"{host.UrlBase}/controlacceso/interno/permisos/{appId}/{usuarioId}");
                    logger.LogDebug($"ProxySeguridad - Respuesta {response.StatusCode} {response.ReasonPhrase}");
                    string contenidoRespuesta = await response.Content.ReadAsStringAsync();
                    if (!response.IsSuccessStatusCode)
                    {
        ...
        try
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    seguridadHttpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", jWT.access_token);
                    HttpResponseMessage response = await seguridadHttpClient.GetAsync($"{host.UrlBase}/controlacceso/interno/roles/{appId}/{usuarioId}");
                    logger.LogDebug($"ProxySeguridad - Respuesta {response.StatusCode} {response.ReasonPhrase}");
                    string contenidoRespuesta = await response.Content.ReadAsStringAsync();
                    if (!response.IsSuccessStatusCode)
                    {
